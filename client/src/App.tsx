...
function App() {
  ...
  useEffect(() => {
    fetchAllData().then(data => setData(data)).catch(e => console.error(e));
  }, []);
  const handleScrapeClick = () => {
    setLoading(true);
    console.log(dataUrl);
    fetch(
      `http://localhost:5000/goods/${ loadSource === Source.Rozetka ? 'rztk' : 'telemart' }`,
      {
        method: 'POST',
        headers: {
          ...
          <Box display={ 'flex' } gap={ 3 } justifyContent={ 'center' }
            ...
            <Box>
              <FormControl>
                ...
                >
                  <FormControlLabel value="ROZETKA" control={ <Radio/> }
                                    label="Rozetka"/>
                  <FormControlLabel value="TELEMART" control={ <Radio/> }
                                    label="Telemart"/>
                </RadioGroup>
              </FormControl>
            </Box>
            <Button variant={ 'contained' }
                    onClick={ handleScrapeClick }>{ isLoading ? 'Loading' : 'Scrape catalog' }</Button>
            <TextField variant={ 'filled' } label={ 'Catalog URL' }
                       value={ dataUrl }
